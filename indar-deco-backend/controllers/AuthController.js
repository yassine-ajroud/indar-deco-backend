    ...
    .then(async user=>{
        if(user){
            ...
            let  mailTransporter= nodemailer.createTransport({
                service:"gmail",auth:{user:"ajroudyassine06@gmail.com",pass:"zyrm prxq djon ugan",}
            }); 
...
const updatepassword = async (req,res)=>{
    ...
    var newrecovery = req.body.newRecovery
    var id = req.body.id
...
//   };
  ...
  const getAllUserEmails = async (req, res, next) => {
    ...
    console.log(emaill);
    try {
      const user = await User.findOne({email:emaill,OAuth:null},{recoveryEmail:1}); // Exclude the password field
...
module.exports = {
    register, login,forgetPassword,Pay,getAllUserEmails,profilgetById,UpdateProfil,Resetpassword,VerifCode,refreshtoken,banUser,getAllUsers,deleteUser,updatepassword
}